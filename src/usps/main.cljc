(ns usps.main
  "Kotodama WASM entrypoint for the Usps clean-room actor (L4) — Clojure port.
  ...
  facts (`usps.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "Filing"      :plural "filings"      :id-prefix "usps_fil"
    :fields [:filerId :form :period :status]     :required [:form :period]
    :coerce {}                       :refs {}}
   {:entity "Entity"      :plural "entities"     :id-prefix "usps_ent"
    :fields [:name :ein :jurisdiction]           :required [:name :ein]
    :coerce {}                       :refs {}}
   {:entity "Dataset"     :plural "datasets"     :id-prefix "usps_dat"
    :fields [:name :agency :updatedAt]           :required [:name :agency]
    :coerce {}                       :refs {}}
   {:entity "Application" :plural "applications" :id-prefix "usps_app"
    :fields [:applicantId :program :status]      :required [:program :status]
    :coerce {}                       :refs {}}
   {:entity "PublicRecord" :plural "publicrecords" :id-prefix "usps_pub"
    :fields [:recordType :jurisdiction :recordedAt] :required [:recordType :jurisdiction]
    :coerce {}                       :refs {}}
   {:entity "Notice"      :plural "notices"      :id-prefix "usps_not"
...
(defn emit-facts
  "EAVT facts for one record: {\"usps.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "usps-compat" :tier tier :entities entities} 200])
